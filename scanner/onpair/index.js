 ...
 *
 * PancakeSwap V2 PairCreated listener
 * - Respects CoreState scanner enable/disable
 * - Handles seenPairs persistently
 * - Emits "newPair" events for the signal pipeline
 */
...
// ----------------------
// Load seen LPs from disk
// ----------------------
async function loadSeenPairs() {
  try {
    const exists = await fs
      .stat(SEEN_PAIRS_FILE)
      .then(() => true)
      .catch(() => false);
...
async function handlePairCreated(token0, token1, pairAddress, event) {
  try {
    // ⚠️ Respect CoreState scannerRunning
    if (!state.scannerRunning) {
      log.debug("Scanner disabled, skipping pair");
      return;
    }
    ...
    const tokenAddress =
      WBNB.includes(a0) && !WBNB.includes(a1)
        ? a1
        : WBNB.includes(a1) && !WBNB.includes(a0)
        ? a0
        : a0;
